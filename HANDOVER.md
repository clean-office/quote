...
- **엑셀 수식 규칙(07-29 수정)**: 모든 수식 셀은 `{formula, result}`로 **계산값을 반드시 함께 기록**(수식만 넣으면 폰 미리보기·수식 미계산 뷰어에서 금액이 전부 0/빈칸으로 깨짐). `wb.calcProperties={fullCalcOnLoad:true}` 유지. 갑지 이미지 크기 고정: 로고 250×51(A~C칸 내, 1~2행 height 20), 명함 125×195(견적조건 11~20행 내). 이 규칙 깨지 말 것.
...
- 07-29: **엑셀 내보내기 깨짐 수정** — 수식 셀에 result 캐시값 추가(뷰어 0 표시 해결), fullCalcOnLoad, 갑지 로고가 제출처 덮던 것·명함이 표 밖으로 나가던 것 크기·위치 조정.
- 08-01: **모노블랙 테마** — 허브 전체 골드→블랙&그레이(:root 변수+하드코딩 색 교체, 상태색 green/red/blue/amber는 유지). **현장 스케줄링**(일정 등록 3단: 종류 칩 work/meet/etc → 현장 select(sites+estimates, 선택 시 주소·전화 자동) → 공정 칩 PROCS 8종. 제목 자동 '현장 — 공정'=schedTitle, 기간 date~end, 달력에 현장별 색 colorFor(해시→SITE_COLORS 8색)·연속 바 ▶ 표시, 현장 상세 모달에 예정 공정 목록) + **일정 달력 뷰**(월간 그리드, 목록↔달력 토글=localStorage mdl_sched_view, 날짜 선택→그날 목록+해당 날짜로 등록, 모바일은 점 표시) + **일정 주소·전화 필드** 추가(mt_addr/mt_phone, 일정 row에서 📍주소→카카오맵, 📞번호→tel: 바로 전화). 견적기 저장 시 클라우드 미연결이면 '이 기기에만 저장' 경고 토스트.
- 08-01: **허브 v2 개편** — ①견적 자동 연동: estimates row+mdl_saves 병합(estPull/estList/estTotal, 계산은 견적기 표준과 동일), 홈 '이번달 견적' KPI·'최근 견적' 패널·견적 탭 금액 표시가 전부 자동. ②영업 파이프라인: STAGES=발송/협의중/계약/시공중/완료 5단계, 견적 저장건은 자동 카드(stage는 DB.pipe[견적이름]에 저장, main row로 동기화), 계약 단계 진입 시 현장 자동 등록 제안. ③가독성 v2 CSS(폰트 확대·대비, 모바일 칸반 가로 스크롤). monthChart 고정날짜 버그 수정. **견적기: '엑셀 불러오기'** (4단계, importXLSX — 자체 엑셀 양식의 내부내역/갑지 파싱 → state 복원, 라운드트립 검증됨. 목표총액은 엑셀에 없으므로 필요시 3단계에서 재입력).
- 08-05: **저장 장애 수정** — seed 덮어쓰기·pull 통짜교체·견적 열기 오작동·저장목록 재정렬 (아래 7장 참고).
- 07-29: **내역서 가로형 페이지 분할** — 내역서가 세로로 길어지던 것을 720×509 고정 가로 페이지로 분할(#detPages 동적 생성, 행 높이 추정 rowH·페이지 예산 320px). 여러 장이면 라벨 `3-1…(1/N)`, 마지막 장에만 직접공사비 계, 중간 장은 "다음 장에 계속". JPG 파일명 `3_내부내역_1..N`(1장이면 기존과 동일).

## 7. 2026-08-05 저장 장애 전수 점검·수정 (중요 — 원인과 대책)
증상: ①관리앱 데이터가 예시(seed)로 되돌아가 있고 일정이 저장 안 됨 ②견적 목록에서 열면 다른 견적이 열림.

원인 4가지와 수정:
1. **신규 기기 seed가 클라우드 실데이터를 덮어씀(치명적)** — 빈 localStorage로 열면 `load()`가 seed 후 `save()`→cloudPush, 이어 `cloudPullMerge()`에서 로컬 _ts가 더 최신이라 판단해 seed를 업로드. 실제로 2026-08-02에 이 경로로 main 행이 예시 데이터로 덮였다. → seed에 `_fresh:true,_ts:0` 표시, `load()`는 더 이상 push 안 함, `cloudPullMerge()`는 `_fresh` 상태면 원격을 그대로 채택.
2. **pull이 로컬 편집을 통째로 날림** — 기존엔 `rt>lt`면 `DB=remote` 통짜 교체. 초기 로드 직후 등록한 일정이 pull 응답에 덮여 사라짐. → `mergeDB()` 도입: sites/sales/customers/dict/tasks/schedule를 **id 단위 합집합 병합**, pipe는 키 병합, 삭제는 `DB._del` 툼스톤으로 전파(delSite·delSale·delMeeting에서 `tomb(id)`). pull 후 `render()` 호출(예전엔 홈에서만 갱신돼 저장 안 된 것처럼 보였음).
3. **견적 "열기"가 항상 같은 걸 염** — hub 견적 목록의 열기 버튼이 `location.href='./index.html'`이라 선택과 무관하게 작성중 draft가 열렸다. → `openEst(k)` → `index.html?open=<이름>`, 견적기 init에서 `loadNamed()`로 해당 견적을 로드 후 URL 정리. 홈 '최근 견적' 행도 클릭 시 열림.
4. **저장목록이 손가락 밑에서 재정렬** — `openSaves()`가 동기 렌더 후 클라우드 pull 완료 시 재렌더 → 누르는 순간 항목이 밀려 다른 게 열림. → pull 끝난 뒤 **한 번만** 렌더(4초 타임아웃), 그 전에는 '동기화 중…' 표시.

그 외: 타이핑마다(autosave) 전체 견적목록을 fetch+upsert하던 것 제거(저장·삭제 시에만 push), 삭제한 견적이 클라우드에서 되살아나지 않게 `mdl_saves_del` 툼스톤 추가, 목록 라벨을 날짜 대신 **이름+금액**으로(같은 현장 여러 버전 구분), cloudPush 3회 재시도, 탭 복귀(visibilitychange) 시 재동기화, `_cloud` 설정을 매번 다시 읽기.

**주의**: 계산 로직·저장 키·시트 디자인은 그대로. 저장목록 금액 표시(`savTotal`)는 표시 전용이며 기존 calc()와 동일 공식.
