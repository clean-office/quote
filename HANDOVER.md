...
- 08-01: **모노블랙 테마** — 허브 전체 골드→블랙&그레이(:root 변수+하드코딩 색 교체, 상태색 green/red/blue/amber는 유지). **일정 주소·전화 필드** 추가(mt_addr/mt_phone, 일정 row에서 📍주소→카카오맵, 📞번호→tel: 바로 전화). 견적기 저장 시 클라우드 미연결이면 '이 기기에만 저장' 경고 토스트.
